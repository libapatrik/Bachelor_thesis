...
for (i in (start + est_periods_OOS):(end-1)) {  
  j <- j + 1    
  actual_ERPlas <- as.numeric(window(ts_df, i+1, i+1)[, dep])
  #1. Historical mean model
  OOS_error_Nlas[j] <- actual_ERPlas - mean(window(ts_df, start, i)[, dep], na.rm=TRUE)
  ...
  set.seed(123)
  cvreg = cv.lars(as.matrix(cbind(XX,XX2,XX3,XX4,XX6,XX7,XX8,XX9,XX10,XX11,XX12,XX13,XX14,XX15,XX16,XX17,XX18)), as.matrix(yy), type="lasso", plot.it=FALSE)
  reg <- lars(as.matrix(cbind(XX,XX2,XX3,XX4,XX6,XX7,XX8,XX9,XX10,XX11,XX12,XX13,XX14,XX15,XX16,XX17,XX18)), as.matrix(yy), type="lasso")
  ...
  newdat_las <- cbind(as.numeric(window(ts_df, i+1, i+1)[, indep]),
                      ...
                      as.numeric(window(ts_df, i+1, i+1)[, indep12]),
  ...
  OOS_error_Alas[j] <-  pred_ERP_las - actual_ERPlas
}
