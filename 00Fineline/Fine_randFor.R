...
for (i in (start + est_periods_OOS):(end-1)) {  
  j <- j + 1    # add
  actual_ERPrf <- as.numeric(window(ts_df, i+1, i+1)[, dep])
  #1. Historical mean model
  OOS_error_Nrf[j] <- actual_ERPrf - mean(window(ts_df, start, i)[, dep], na.rm=TRUE)
  ...
  # RandFor Model
  set.seed(123)
  rf_OOS <- randomForest(yy ~ XX+XX2+XX3+XX4+XX6+XX7+XX8+XX9+XX10+XX11+XX12+XX13+XX14+XX15+XX16+XX17+XX18)
  ...
  OOS_error_Arf[j] <-  pred_ERP_rf - actual_ERPrf
}
